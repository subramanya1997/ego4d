...
def get_dataloader(args):
    ...
    val_nlq = Ego4d_MQ(args.input_val_split, modalities=modals, split="val", save_or_load_path=f"{args.dataloader_cache_path}/sample_mq_val.pkl", config_file = args.dataloader_config)
...
def test_model(model, dataloader, model_loss, args, writer, epoch, Test = False):
    ...
    for i, data in enumerate(tqdm_obj):
        ...
        s_idx = starts.shape[-1]-1 if torch.sum(starts) == 0 else np.where(starts.cpu().numpy() == 1)[-1][0]
        ...
        records.append({"sample_id": int(sample_id[0]), 
                        ...
                        "GT_starts": int(s_idx), #int(np.where(starts.cpu().numpy() == 1)[-1][0]),
                        "GT_ends": int(end_idx),
...
if __name__ == "__main__":
    ...
    # print("query feature size ", args.query_feature_size)
