...
class MEME_LOSS(nn.Module):
    ...
    def forward(self, pred, target_start, target_end, target_in_range,loss_type='hard_qa'):
        if loss_type == 'hard_qa':
            output = self.em_joint_loss(pred, target_start, target_end, target_in_range)
        if loss_type == 'soft_qa':
    ...
    def qa_soft_loss_backup(self, pred, target_start, target_end, target_in_range):
        '''
        Compute the QA style loss with soft boundaries
        ...
        '''
        #soften the bondaries using gaussian filter
        target_start = target_start.unsqueeze(0).unsqueeze(0).to(float)
        target_end = target_end.unsqueeze(0).unsqueeze(0).to(float)
        filter = torch.tensor(scipy.signal.windows.gaussian(self.boundary_smoothing, std=1)).unsqueeze(0).unsqueeze(0).to(float).to(pred.device)
        padding = int((self.boundary_smoothing -1) / 2)
        target_start = F.conv1d(target_start, filter, padding=padding).squeeze(0).squeeze(0)
        target_end = F.conv1d(target_end, filter, padding=padding).squeeze(0).squeeze(0)
        # target_start = F.softmax(target_start, dim=1)
        # target_end = F.log_softmax(target_end, dim=1)
        ...
        loss = -torch.sum(F.log_softmax(pred[:, 0]) * target_start)*0.5
        loss += -torch.sum(F.log_softmax(pred[:, 1]) * target_end)*0.5
        return torch.mean(loss)

    def ner_loss():
        '''
        Compute the NER style loss with tag prediction
        '''
        pass