...
def init_custom_model(folder_path="output/models/model1", model_name= "meme"):
    my_config = RobertaConfig(num_hidden_layers=4, max_position_embeddings=512,type_vocab_size=6)
    my_config.save_pretrained(save_directory=folder_path)
    my_config = RobertaConfig.from_pretrained(f"{folder_path}/config.json")

    model = RobertaModel(my_config)
...
def label_windows(is_ans):
    ...
    mask = is_ans!=-100
    masked = is_ans * mask.to(torch.int)
    has_ans = masked.sum(dim=1)
    labels = has_ans > 0
    return labels
