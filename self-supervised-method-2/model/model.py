...
class MEME_SS_BASE(nn.Module):
    ...
    def forward(self, video, text, text_length, audio, args, modalities=None, **kwargs):
        ...
        input_, clsTokens = self.create_model_input(video, audio, text, text_length, args, modalities)
        ...
        frame_number_pred_1 = frame_number_pred[0][ clsTokens['Query'] ,0]

        if frame_number_pred_1.shape[0] == 0:
            print('pred input is not same as output')
            print(f'{input_.shape} {frame_number_pred.shape} {output[0]} {clsTokens["Query"] }')
        
        
        reorder_pred = None

        return output, frame_pred, reorder_pred, frame_number_pred_1, clsTokens
