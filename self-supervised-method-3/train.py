...
def train(model, dataloader, model_loss, optimizer, args, writer, epoch):
    ...
    for data in tqdm_obj:
        ...
        if iter%20 == 0:
            ...
            print(f'Total/train/{epoch} : {total_loss} ==> iter : {iter}')
        ...
        wandb.log({"Batch Loss/train": loss.detach().cpu().item(), f"Total loss/train": total_loss, "Epoch":epoch } ) 
...
def test_model(model, dataloader, model_loss, args, writer, epoch, Test = False):
    ...
    for i, data in enumerate(tqdm_obj):
        ...
        if iter%20 == 0:
            ...
            print(f'Total/{split}/{epoch} : {total_loss} ==> iter : {iter}')
        ...
        wandb.log({f"Batch Loss/{split}": loss.detach().cpu().item(), f"Total loss/{split}": total_loss, "Epoch":epoch})
...
def save_checkpoint(model, optimizer, epoch, loss, acc=None, acc5=None, acc10=None, path=None):
...
if __name__ == "__main__":
    ...
    for epoch in range(0, args.num_epochs):
        ...
        if best_val_loss > val_loss:
            ...
            save_checkpoint(model, optimizer, epoch, val_loss, path=args.best_model_path)
        
        save_checkpoint(model, optimizer, epoch, val_loss,path=args.last_model_path)
